...
module.exports.createCaptions = async (config) => {
    if (fs.existsSync(config.datadir + '/captions.json')) {
        const captions = JSON.parse(fs.readFileSync(config.datadir + '/captions.json').toString())
...
const transformCaptions = async (caption, config) => {
    ...
    const captionInFile = config.datadir + '/caption_' + caption.locale + '.vtt'
